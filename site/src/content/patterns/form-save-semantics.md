...
form extensions, imported from the shared barrel and attached with `.extend()`,
...
everything. The form must stop reading as dirty, but without adopting edits
the user typed while the request was in flight. That is `withSavedState`: the
